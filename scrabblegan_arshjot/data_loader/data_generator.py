...
class DataLoader:
    ...
    def batch_collate(self, batch):
        ...
        items['img'] = pad_sequence([item['img'].squeeze(0).permute(1, 0) if item['img'].squeeze(0).dim() == 2 else item['img'].unsqueeze(0).squeeze(0).view(1,-1).permute(1,0) for item in batch],
                                    batch_first=True,
                                    padding_value=1.)
