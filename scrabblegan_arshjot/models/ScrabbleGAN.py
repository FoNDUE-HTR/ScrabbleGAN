...
class ScrabbleGAN(nn.Module):
    def __init__(self, cfg, char_map):
        ...
        import pandas as pd
        if cfg.dataset == 'IAM':
            self.fake_words = pd.read_csv(cfg.lexicon_file, sep='\t')['words']
            self.fake_words = self.fake_words.loc[self.fake_words.str.len() < 20]
            self.fake_words = self.fake_words.to_list()
        else:
            exception_chars = ['ï', 'ü', '.', '_', 'ö', ',', 'ã', 'ñ']
            df = pd.read_csv(cfg.lexicon_file, sep='\t')
            col = 'lemme' if 'lemme' in df.columns else df.columns[0]
            self.fake_words = df[col].dropna().astype(str).tolist()
            self.fake_words = [w for w in self.fake_words
                               if all(char not in w for char in exception_chars)]

        # On filtre les mots valides (tous les caractères doivent être dans char_map)
        self.fake_words = [
            word for word in self.fake_words
            if word and all(c in char_map for c in word)
        ]

        # Distribution catégorielle : uniforme si on a des mots, vide sinon
        if self.fake_words:
            self.fake_y_dist = torch.distributions.Categorical(
                logits=torch.zeros(len(self.fake_words))
            )
        else:
            # Distribution vide - attention : sampler dessus va planter
            # → tu peux aussi mettre self.fake_y_dist = None
            # et gérer le cas dans forward_fake / sample si besoin
            self.fake_y_dist = torch.distributions.Categorical(
