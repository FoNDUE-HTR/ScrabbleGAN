...
class ModelCheckpoint:
    ...
    def save(self, model, epoch, G_opt, D_opt, R_opt, G_sch=None, D_sch=None, R_sch=None, r_real=None):
        ...
        # Garder le meilleur modele selon R_real
        if r_real is not None:
            best_path = os.path.join(self.weight_dir, 'model_best.pth.tar')
            is_best = True
            if os.path.exists(best_path):
                best = torch.load(best_path, map_location='cpu', weights_only=False)
                if best.get('r_real') is not None and r_real >= best['r_real']:
                    is_best = False
            if is_best:
                shutil.copy(filename, best_path)
                print(f'  -> Meilleur modele sauvegarde (R_real={r_real:.3f}) -> model_best.pth.tar')
